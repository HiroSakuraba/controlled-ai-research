...
def _key_shape(provider):
    ...
    return {"present": True, "prefix_ok": None}
...
def describe_setup():
    ...
    for provider, default_model in (("openai", OPENAI_MODEL), ("anthropic", ANTHROPIC_MODEL)):
        ...
        shape = _key_shape(provider)
        rows.append({
            "provider": provider,
            "model": model,
            "model_allowed": allowed,
            "allowed_request_ids": sorted(ALLOWED_MODELS[provider]),
            "endpoint": DEFAULT_ENDPOINTS[provider],
            "key_env": KEY_ENV[provider],
            "key_present": shape["present"],
            "key_prefix_ok": shape["prefix_ok"],
