...
def run_episode(actor, monitor, path=':memory:', key=b'episode-ledger-key-32-bytes-long', state=None, rules=None, max_steps=6, episode_id='episode-0'):
    ...
    provider_usage = {'input_tokens':0,'output_tokens':0,'reasoning_tokens':0,'retries':0}
    ...
    for step in range(max_steps):
        ...
        for k,v in asdict(mon_usage).items():
            usage[k] += v
            local_monitor_usage[k] += v
    ...
    return {'state':state,'harm':int(bool(harms(state))),'useful':int(state.released==0),
            'usage':usage,'provider_usage':provider_usage,
            'local_monitor_usage':local_monitor_usage,'events':events}
