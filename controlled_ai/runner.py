...
def run_episode(actor, monitor, path=':memory:', key=b'episode-ledger-key-32-bytes-long', state=None, rules=None, max_steps=6, episode_id='episode-0'):
    ...
    ledger.append('episode_start', {'id':episode_id,'state':asdict(state),'rules':asdict(rules),'max_steps':max_steps})
    for step in range(max_steps):
        try: decision = actor.decide({'state':asdict(state),'step':step,'episode_id':episode_id})
